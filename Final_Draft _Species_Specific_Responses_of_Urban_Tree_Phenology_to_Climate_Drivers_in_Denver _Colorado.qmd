...
csl: ./literature_review/chicago-author-date.csl
editor: visual
execute:
  echo: false
  warning: false
...
An interactive Shiny application was developed to facilitate sample selection and quality control, allowing visual inspection of candidate trees' spatial distribution across Denver County \[#git_report_link\]. The final sample consisted of 23,779 individual trees from 15 species across 11 genera. The sample included 11 native species, 2 non-native species, and 2 invasive species (@tbl-species). The sample data set can be found here along with the corresponding cleaning code\[#git_report_link\].
...
where $mn$ and $mx$ are the minimum and maximum NDVI values, $t$ is time, $(m_3', m_5')$ are the start date of the gowning season (SOS) and end of growing season (EOS), respectively, and $(m_4', m_6')$ the slope of spring greening up or autumn senescence onset.

After testing multiple curve types, the Elmore curve was selected because it consistently produced the most reliable fits across species and provided biologically interpretable parameters.

Curve-fit quality was assessed using adjusted R², with trees retained in the final sample only if they met two criteria: (1) mean R² ≥ 0.90 across all fitted growing seasons, and (2) at least six growing seasons successfully detected over the seven-year study period. These conservative thresholds ensured high-quality phenological curves while excluding trees with poor NDVI signal. Four phenological stage dates were extracted from each annual curve using the inflection point method [@ZHANG2003], which identifies the dates of maximum rate-of-change in the NDVI trajectory: (1) Greenup, the spring inflection point marking the beginning of leaf emergence, (2) Maturity, the date at which canopy development has reached a leveling-off point in the spring, (3) Senescence, the fall inflection point marking the start of leaf-loss, and (4) Dormancy, the date when a tree has lost its leaves and NDVI begins to level off for the winter. Inflection points were chosen over alternative methods (e.g., threshold-based or derivative-based) because they provide consistent timing estimates across species with different canopy densities.
...
sample_data <- read_csv("./tables/final_sample.csv") %>%
  select(-`...1`) %>%
  rename(Species = species_common, 
         `Leaf Persistence` = leaf_persistence,
         Observations = n) %>%
...
total_row <- tibble(
  Species = "Total",
  `Leaf Persistence` = "",
  Observations = sum(sample_data$Observations)
)

bind_rows(sample_data, total_row) %>%
  knitr::kable(format = "markdown",  # Add this line
               booktabs = TRUE,
               align = c("l", "l", "r"),
...
As in similar studies of land-cover effects on tree penology conducted in Washington, D.C. [@Alonzo_2021; @ALONZO2023], circular buffers of 90 meters were created around each tree's GPS coordinates. Within each buffer, the number of pixels belonging to each land-cover class was tallied and converted to percentages, representing the proportion of the surrounding landscape occupied by each cover type. Following methodology similar to [@Crawford_2025], the nine original land cover classes were consolidated into five ecologically meaningful categories to simplify interpretation and reduce model complexity (@tbl-lc). The 90-meter buffer land cover variables were used as predictors in all XGBoost models to account for local environmental context effects on phenological timing.
...
To evaluate phonological responses to climate change, four warming scenarios, outlined in the @Bolinger2023 were modeled using species and phenogocal stage Boosted models. A +0.8°C increase scenario, representing the observed warming trends from the past 30 years, plus three 1.8°C increase scenario with three precipitation variants: moderate (no change), dry (-7% precipitation), and wet (+7% precipitation). Given the limitations of our data, 1.8°C is the highest scenario we can model within the expected 1.4°C to 3°C increase in average temperature by mid-centruy. In addition a Baseline scenario was run through the model using 1970-2000 climate normal predict phenological timing under unalterd climate conditions. For each scenario, predictions were generated for all species while holding land cover variables constant at their mean values, allowing us to isolate the effects of temperature and precipitation changes on phenological timing.

Climate warming scenarios revealed substantial phenological shifts across all species, with the most pronounced changes occurring in autumn phenology (@tbl-climate-scenarios, @fig-climate-scenarios). Current warming scenario (+0.8°C), the species models predicted that average growing season ($dormancy - greenup$) for all species has lengthened by 8.2 days over the 1970-2000 baseline. The change has been primarily driven by delayed dormancy and senescence rather than greenup and matruity timing. Senescence was delays 3.6 days and dormancy delays 8.1 days.

Species responses under the +0.8°C scenario varied substantially, revealing differential adaptation trajectories. Tree of Heaven, Common Hackberry, Honely Locus, and Western Catalpa showed the most dramatic responses, with growing seasons extending by 11-16 days through substantial dormancy delays. In contrast, Northern Red Oak, Green Ash, Siberian Elms, and cottonwoods exhibited modest growing season changes (-0.5 to 6.2 days), maintaining relatively conservative phenological timing despite almost three decades of warming. Peak canopy duration ($Senescence timing - Maturity timing$) under the current warming ranged from 0 days (Common Hackberry) to 16.9 days (Siberian Elm), with most species showing on average, 4.4 adtional days of peak canopy coverage. Interestingly, Red Oak and Siberian Elms, had some of the lowest changes in growing seasons, but had some of the highest changes in peak canopy timing, increasing from 8.7 and 16.9 days respectively. The combined effect of stable greenup, modestly advanced maturity, delayed senescence, and strongly delayed dormancy suggests that the past 30 years of warming have already substantially extended both the period of active canopy cover and the total growing season length for Denver's urban forest.

Under the +1.8°C warming scenarios, growing season length is projected to increase by 9.7 days over the baseline and peak canopy duration extends by 10.9 days under moderate moisture conditions. Under the +1.8°C wet scenario, the 7% additional precipitation will not increase the growing season over the moderate scenario, but peak canopy duration is expected to increase by 14.1 days. The +1.8°C scenario with -7% precipitation showed minimal additional effects beyond temperature alone. The rate of change in the growing season length, form less than 1 day, from observed warming of +0.8°C to the projected +1.8°C, compared to the rate of change from the baseline to current warming suggests that phonological responses to climate change may have diminishing relationship, possibly indicating physiological or environmental constraints.

As growing season begins to slow or stop increasing, peak canopy begins to increase. This phenomenon can be seen in Common Hackbeery, American Elm, Honey locust and Tree of Heaven from current growing scenario to +1.8°C scenarios, and in the Red Oaks and Siberian Elms from the baseline to the +0.8°C scenario. However average temperatures continure to raise, the rate of peak canopy growth Red Oaks and Siberian Elms. This suggest that diminishing returns also seem to apply to peak canopy length.

Across all warming scenario, green up timing remained largely unchanged with most species (changes \<1 day for 10 of 12 species), with only Western Catalpa accurately being delaying by 1 day and the Common Hackberry greening up earlier by a 1.8 days. 4 Discussion
