...
plot_annual_curve(seasons[[1]], show_kprime = TRUE, show_transitions = TRUE)

```


```{r}
pheno_metrics <- function(location){
  extracts %>%
    filter(UID == location) %>% 
    mutate(ndvi = despike(ndvi, "median", n = 2, k = 5)) -> extracts_samp
...
for (i in seq_along(uids)) {
  current_metrics <- pheno_metrics(i)
