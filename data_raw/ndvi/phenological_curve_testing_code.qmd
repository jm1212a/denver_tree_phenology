...
```{r}
plot_annual_curve <- function(growing_seasons, ndvi_obs, boot_int = 50, show_ci = TRUE, ci = 0.95,
...
#plot_annual_curve(seasons[[4]], ndvi_obs = ndvi_normalized , show_kprime = TRUE, show_transitions = TRUE) #use example
...
# list() -> results_list 
# 
# progress_bar$new(format = "Processing (:spin) [:bar] :percent eta: :eta",
#                  total = length(uids),
#                  width = 60) -> progress
# 
# 
# for (i in seq_along(uids)) {
#   pheno_metrics(uids[i], ndvi_data = normalized_ndvi_extracts) -> current_metrics 
#   as.data.frame(current_metrics) -> results_list[[i]]
#   progress$tick()
# }
# 
# bind_rows(results_list) -> sample_extracts

map_df(uids, ~ pheno_metrics(.x, ndvi_data = normalized_ndvi_extracts), 
       .progress = TRUE ) -> sample_extracts
...
sample_extracts %>% 
  ...
  mutate(kept_per = n_kept / n_proceed,
         proc_per = n_proceed / length(uids))
...
plot_growing_season(extracts_samp, ndvi_obs = ndvi_normalized, maxtroughhight = .6) #use example 

fit_annual_curve(seasons[[2]], boot_int = 100, ci = .95, ndvi_obs = ndvi_normalized) 

plot_annual_curve(seasons[[2]], ndvi_obs = ndvi_normalized , show_kprime = FALSE, show_transitions = TRUE, boot_int = 100, ci = .99) #use example
