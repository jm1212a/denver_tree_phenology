...
# Code for processing - mean R2 \> .9 and 6 season for each tree

```{r}

batch_size <- 20
output_dir <- "./pheno_curve_metrics/batches_.9"
...
write_rds(all_results, "./pheno_curve_metrics/extracted_metrics_.9_test.rds")
...
# Code for processing - R2 \> .9 for each curve
...
set_options(season = list(
  r_max = 0.05,
  r_min = 0.01,
  calendarYear = TRUE
  ))

# if (phenofit:::.options$season$ypeak_min < 0.05) {
#   stop("set_options did not work — ypeak_min is still below 0.2")
# }
# 
# if (phenofit:::.options$season$r_max < 0.05) {
#   stop("set_options did not work — r_max is still below 0.3")
# }
# 
# if (phenofit:::.options$season$r_min < 0.01) {
#   stop("set_options did not work — r_min is still below 0.15")
# }
...
process_tree <- function(tree_data, trees, total_trees, p = NULL) {
  ...
  tryCatch({
    ...
    # Preprocess NDVI
    tree_data %>% 
      mutate(ndvi = despike(ndvi, "median", n = 2, k = 5)) %>%
      mutate(
        time_numeric = as.numeric(im_date),
        trend = predict(lm(ndvi ~ time_numeric)),
        ndvi_detrended = ndvi - trend + mean(ndvi, na.rm = TRUE)
    ...
    if (!is.null(brks_mov$dt)) {
      brks_mov$dt <- brks_mov$dt[brks_mov$dt$season == 1, ]
    }
    
    # Fit phenology curves
    fit <- curvefits(x_detrended, brks_mov)
    ...
    if (nrow(good_seasons) >= 1) {
      
      # Extract phenology metrics
      metrics <- get_pheno(fit, "Elmore")
      
      good_flags <- good_seasons$flag 
      
      metrics_date <- metrics$date$Elmore %>% 
        mutate(UID = i) %>%
        filter(flag %in% good_flags)  
      ...
      season_curve <- tibble(
        UID = i,
        season_data = list(x_detrended))
      
      # Join fit objects to statistics
      stats <- stats %>%
        mutate(UID = i) %>%
        filter(meth != "Elmore" | flag %in% good_flags) %>%  # drop bad Elmore seasons
        left_join(season_curve, by = "UID") %>%
        left_join(fit_lookup, by = "flag")
...
read_rds("./pheno_curve_metrics/extracted_metrics_.9_test.rds") -> test_data

test_data %>% 
  filter(flag != "2025_1") -> test_data

test_data %>% 
  group_by(flag) %>% 
  summarise(n = n()) %>% 
  ggplot(aes(x = flag, y = n)) +
  geom_col()
...
plot_year(test_data, "2020_1", "10160", metrics = "Inflection")

plot_year(test_data, "2021_1", "10160", metrics = "Inflection")

test_data %>% 
  filter(UID == "6004") -> test_data

plot_data <- data.frame(
  t = as.Date(test_data$season_data[[1]]$t, origin = "1970-01-01"),
  y = test_data$season_data[[1]]$y
)

ggplot(plot_data, aes(x = t, y = y)) +
  geom_point()

#plot_year(test_data, "2020_1", "6004", metrics = "Inflection")

#plot_year(test_data, "2021_1", "6004", metrics = "Inflection")

fit_stats %>% 
  group_by(flag, meth) %>% 
  summarise(mean_r = mean(R2)) %>% 
  filter(meth == "Elmore")
...
extracts %>%
  group_by(UID) %>% 
  summarise(min_ndvi = min(ndvi)) %>%
  ggplot(aes(x = min_ndvi)) +
  geom_histogram(bins = 30) +
  scale_x_continuous(limits = c(-1, 1)) +
  labs(x = "Minimum NDVI", y = "Count", title = "Distribution of Minimum NDVI by Tree")

```
```{r}

uid_sample <- c(11763, 19630, 3330, 924, 5412)

# extracts %>%
#   filter(UID %in% uid_sample) %>% 
#   write.csv("./pheno_curve_metrics/extracts_uid_sample.rds")

read.csv("./pheno_curve_metrics/extracts_uid_sample.rds")
...
extracts %>%
  filter(UID == "924") %>%
  mutate(year = lubridate::year(im_date),
         doy = lubridate::yday(im_date)) %>%
