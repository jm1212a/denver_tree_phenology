...
read.csv("./raw_extracts/lc_fracs_by_circle_FULL.csv") %>% 
  rename(structures = class_1,
         ...
         cropland = class_8,
         scrubland = class_9) %>% 
  mutate(unirrigated_90m = rowSums(across(c(prairie_grassland, scrubland)), 
                               na.rm = TRUE),
         irrigated_90m = rowSums(across(c(cropland, irrigated_land)), 
                             na.rm = TRUE),
         impervious_90m = rowSums(across(c(structures, impervious_surfaces, barren_rock)), 
                              na.rm = TRUE),
...
summary(lc_90$impervious_90m)

lc_90 %>% 
  slice_max(impervious_90m, n = 1)
