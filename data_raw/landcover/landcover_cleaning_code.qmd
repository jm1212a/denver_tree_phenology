...
read.csv("./raw_extracts/lc_fracs_by_circle_FULL.csv") %>% 
  rename(structures = class_1,
         ...
         cropland = class_8,
         scrubland = class_9) %>% 
  mutate(unirrigated = rowSums(across(c(prairie_grassland, barren_rock, scrubland)), 
                               na.rm = TRUE),
         irrigated = rowSums(across(c(cropland, irrigated_land)), 
                             na.rm = TRUE),
         impervious = rowSums(across(c(structures, impervious_surfaces)), 
                              na.rm = TRUE)) %>% 
