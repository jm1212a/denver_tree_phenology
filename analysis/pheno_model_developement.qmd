...
all_cors %>% 
  ...
  mutate(
    ...
    pheno_var = pheno_labels[pheno_var]  # Apply labels
  ) %>% 
  bind_rows(
    all_cors %>% 
      pivot_longer(cols = -c(pheno_var, climate_var), 
      ...
      summarise(correlation = mean(correlation, na.rm = TRUE), .groups = "drop") %>% 
      mutate(
        pheno_var = pheno_labels[pheno_var],  # ADD THIS LINE
        species = "Mean", 
        group = "Mean"
      )
  ) %>% 
...
plot_year(tree_ldc_metrics_climate, "2021", "10006", metrics = c("Inflection","DER.pos"))

tree_ldc_metrics_climate$season_data[[1]]

plot(tree_ldc_metrics_climate$season_data[[1]]$t, tree_ldc_metrics_climate$season_data[[1]]$y)
