...
climate_vars <- list(
  Greenup_doy = c("late_winter_temp", "late_winter_prcp", "late_winter_mean_tmax","impervious_90m", "irrigated_90m"),
  Maturity_doy = c("late_spring_temp", "spring_prcp", "late_spring_mean_tmax", "impervious_90m", "irrigated_90m"), 
  Senescence_doy = c("late_summer_prcp", "early_fall_mean_tmax", "early_fall_temp", "impervious_90m", "irrigated_90m"),
  Dormancy_doy = c("fall_cumulative_prcp", "fall_mean_tmax", "late_fall_temp", "impervious_90m", "irrigated_90m")
)
...
landcover_vars <- c("impervious_90m", "irrigated_90m")
...
scenario_warm_wet <- scenario_warm %>%
  mutate(across(all_of(prcp_vars), ~.x * 1.1))

# +2°C - 10% less precipitation
scenario_warm_dry <- scenario_warm %>%
  mutate(across(all_of(prcp_vars), ~.x * 0.9))
...
predictions_warm_wet <- predict_climate_scenario(xgb_models, scenario_warm_wet, "+2C +10% precip")
predictions_warm_dry <- predict_climate_scenario(xgb_models, scenario_warm_dry, "+2C -10% precip")
...
all_scenario_predictions %>% 
  ...
  filter(species == "American Linden",
         scenario %in% c("Baseline", "+2C"))
