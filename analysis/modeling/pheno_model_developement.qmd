...
climate_vars <- list(
  Greenup_doy = c("late_winter_temp", "late_winter_prcp", "impervious_90m", "irrigated_90m"),
  ...
  DER.pos_doy = c("late_spring_cumulative_prcp", "spring_temp", "late_spring_mean_tmax","impervious_90m", "irrigated_90m")
)
...
fit_xgboost_model <- function(species_name, pheno_var, climate_predictors, data) {
  ...
  xgb_data <- xgb_data %>%
    ...
    mutate(
      ...
      tree_n_obs = n(),
      temp_precip_interaction = if("temperature" %in% climate_predictors && 
                                   "precipitation" %in% climate_predictors) 
                                temperature * precipitation else 0
    ) %>%
    ungroup() %>%
...
xgb_table %>% 
  ...
  summarize(mean_RMSE = mean(RMSE),
            ...
            outlier_impact = (mean_RMSE^2 - mean_MAE^2) / mean_MAE^2)
...
for (result_name in names(xgb_results)) {
  result <- xgb_results[[result_name]]
  
  # Get the actual data
  actual_data <- tree_ldc_metrics_climate %>%
    filter(species_common == result$species, year < 2024) %>%
    pull(result$phenology) %>%
    na.omit()
  
  # Calculate baseline metrics
  data_sd <- sd(actual_data)
  data_range <- diff(range(actual_data))
  
  cat("\n", result$species, "-", result$phenology, "\n")
  cat("RMSE:", round(result$cv_rmse, 2), "days\n")
  cat("Data SD:", round(data_sd, 2), "days\n")
  cat("Data Range:", round(data_range, 2), "days\n")
  cat("RMSE/SD ratio:", round(result$cv_rmse / data_sd, 3), "\n")
  cat("R²:", round(result$cv_r2, 3), "\n\n")
}
