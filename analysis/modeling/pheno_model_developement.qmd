...
fit_xgboost_model <- function(species_name, pheno_var, climate_predictors, data) {
  ...
  # Calculate CV metrics
  cv_r2 <- cor(y, predictions)^2
  cv_rmse <- sqrt(mean((y - predictions)^2))
...
xgb_table <- map_df(xgb_results, function(x) {
  data.frame(
    ...
    n_obs = x$n_obs,
    xgb_cv_r2 = x$cv_r2,
    xgb_cv_rmse = x$cv_rmse
  )
})

# Compare to mixed models
comparison_xgb <- results_table %>%
  select(species, phenology, 
         mixed_cv_r2 = cv_r2_conditional,
         mixed_cv_rmse = cv_rmse_conditional) %>%
  left_join(xgb_table, by = c("species", "phenology")) %>%
  mutate(
    # Improvements
    r2_improvement = xgb_cv_r2 - mixed_cv_r2,
    rmse_improvement = mixed_cv_rmse - xgb_cv_rmse,  # Positive = XGBoost better
    
    # Which model wins
    xgb_wins = xgb_cv_r2 > mixed_cv_r2
  )
...
mean(comparison_xgb$xgb_cv_rmse)

mean(comparison_xgb$xgb_cv_r2)

mean(comparison_xgb$mixed_cv_rmse)

mean(comparison_xgb$mixed_cv_r2)
