...
all_cors %>% 
  ...
  bind_rows(
    all_cors %>% 
      ...
      pivot_longer(cols = c(mean_abs_cor, sd_cor), 
                   ...
                   values_to = "correlation") %>% 
      mutate(
        pheno_var = "Total",  # Changed from "Overall"
  ...
  mutate(pheno_var = case_when(
    pheno_var == "Total" ~ pheno_var,  # Changed from "Overall"
    TRUE ~ pheno_labels[pheno_var]
  )) %>%
  mutate(pheno_var = factor(pheno_var, 
                            levels = c("Green-up", "Maturity", "Senescence", "Dormancy", "Total"))) %>%  # Changed from "Overall"
  ggplot(aes(x = species, y = climate_var, fill = correlation)) +
  geom_tile(color = "white", linewidth = 0.5) +
...
pd_all_species <- map_df(species_list, function(sp) {
  tryCatch({
    get_partial_dependence(
      species_name = sp,
      phenology_stage = "Greenup_doy",
      pred_var = "late_winter_prcp",
...
ggplot(pd_all_species, aes(x = late_winter_prcp, y = yhat, color = species)) +
  geom_smooth(size = 1) +
  labs(
    x = "Late Winter Prcp. (°C)",
    y = "Predicted Greenup DOY",
    title = "Greenup Response to Temperature Across Species",
    color = "Species"
  ) +
  theme_minimal() +
...
pd_all_results %>%
  ...
  arrange(pheno_order, var_type, variable) %>%  # Sort by phenology, then var_type (Precip first), then variable name
  mutate(var_label = factor(var_label, levels = unique(var_label))) %>%
  ggplot(aes(x = x_value, y = yhat, color = species)) +
  geom_smooth(se = FALSE) +
  scale_color_brewer(palette = "Paired") +
  facet_wrap(~var_label, scales = "free", nrow = 6, ncol = 2) +  # Changed to nrow=6 to accommodate grouping
  labs(
    x = NULL, 
